...
function LoungeChatScreen({navigation, route}) {
  const [profileModalVis, setprofileModalVis] = useState(false);
  const {LoungeName, senderBlockchainAddress} = route.params;
  //   const LoungeName = '1';
  ...
  const loadChats = async () => {
    try {
      const response = await fetch(
        `https://02ba-103-175-180-42.in.ngrok.io/Lounges/${LoungeName}/messages`,
        {
          method: 'GET',
          headers: {
  ...
  return (
    <View style={{width: windowWidth, height: windowHeight, flex: 1}}>
      <Modal
        ...
        <MembersModal LoungeName={LoungeName} blockAddSender={blockAddSender} />
      </Modal>
...
export default LoungeChatScreen;
